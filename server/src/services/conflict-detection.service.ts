...
export async function detectConflict(
  ...
  try {
    ...
    if (!raw) {
      console.warn(
        '[detectConflict] LLM returned empty response — using safe default.',
      );
      return SAFE_DEFAULT;
    }
...
function parseConflictResponse(raw: string): ConflictAnalysis {
  try {
    ...
    if (typeof parsed !== 'object' || parsed === null) {
      console.warn(
        '[detectConflict] LLM returned non-object JSON — using safe default.',
      );
      return SAFE_DEFAULT;
    }
    ...
    const isConflict =
      typeof obj['is_conflict'] === 'boolean' ? obj['is_conflict'] : false;
    const rawConfidence =
    ...
    console.warn(
      '[detectConflict] Failed to parse LLM response as JSON:',
      raw.slice(0, 200),
    );
    return SAFE_DEFAULT;
  }
  ...
  switch (strategy) {
    ...
    case 'merge': {
      try {
        ...
        const mergedText =
          typeof parsed['merged_text'] === 'string'
            ? parsed['merged_text']
            : null;
        if (!mergedText)
          throw new Error('LLM response missing merged_text field');
        ...
        const msg = err instanceof Error ? err.message : 'unknown error';
        console.warn(
          '[resolveConflict] merge LLM call failed, falling back to recency:',
          msg,
        );
        return {
          action: 'replace',
...
export async function filterRetrieved(
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : 'unknown error';
    console.warn(
      '[filterRetrieved] error during deduplication, returning input unchanged:',
      msg,
    );
    return memories;
  }
...
export async function checkBeforeStore(
  ...
  const analyses = await Promise.all(
    similar.map((candidate) =>
      limit(async () => {
        const analysis = await detectConflict(
          incoming.text,
  ...
  const confirmedConflicts = analyses.filter(
    ({ analysis }) =>
      analysis.isConflict &&
      analysis.confidence >= CONFLICT_CONFIDENCE_THRESHOLD,
  );
